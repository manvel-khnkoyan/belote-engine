...
class SoftPlayerAgent(Agent):
    """Soft player agent that always plays the lowest card available"""
    
    def choose_action(self, state: State, actions: List[Action]) -> Tuple[Action, Dict[str, Any] | None]:
        """Choose the lowest ranked card from valid actions"""
        ...
        # Find the card with the lowest rank value
        lowest_card_action = min(play_actions, key=lambda action: (action.card.rank, action.card.suit))
        
        return lowest_card_action, None
